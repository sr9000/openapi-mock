...
1. **Создайте OpenAPI спецификацию** в директории `specs/<api-name>/openapi.yaml`.
   ...
   ```bash
   mkdir -p specs/myservice
   # Создайте specs/myservice/openapi.yaml с вашей спецификацией
   ```
...
2. **Запустите команду сборки**:
   ...
   Эта команда выполнит:
    - `make openapi` — генерация Go-кода из всех спецификаций в `specs/`
...
* **Мульти-API поддержка**: Добавляйте любое количество OpenAPI спецификаций в `specs/`. Все API автоматически
