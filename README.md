...
├── pkg/
│   ├── observability/   # Контекстные ключи, логгер, трейсинг
...
* **Ключи контекста**:
  Вы можете получить сгенерированный Request ID внутри ваших заглушек, используя хелпер из `pkg/observability`.
...
### Core-эндпоинты (обязательные, идентичны grpc-mock):
